...
static uintptr_t syscall(uintptr_t which, uint32_t arg0, uint32_t arg1, uint32_t arg2)
{
  ...
  case SYS_write:
    ...
    for(int i = 0; i < arg2; i++){
      //while(UART_IDLE != 0); //Wait for uart to go idle

      UART_OUT = ((unsigned char *)( arg1))[i];
    }
    break;
...
/*
  volatile uint32_t magic_mem[8] __attribute__((aligned(32)));
  magic_mem[0] = which;
  magic_mem[1] = arg0;
  magic_mem[2] = arg1;
  magic_mem[3] = arg2;
...
void printstr2(const char* s, int len){
  UART_START = (int)s;
  UART_END = (int)s + len;
}
