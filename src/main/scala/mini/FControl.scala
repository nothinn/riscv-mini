...
object FControl {
  ...
  val map = Array(
    ...
    FMUL_S  -> List(FALU_MUL  , N, F_ST_XXX, F_LD_XXX, F_WB_ALU, Y, N),

    FLW     -> List(FALU_XXX  , Y, F_ST_XXX, F_LD_LW , F_WB_MEM, Y, N),

    FCVT_W_S-> List(FALU_CNV_W, N, F_ST_XXX, F_LD_XXX, F_WB_REG, N, N),
    FCVT_WU_S->List(FALU_CNV_WU,N, F_ST_XXX, F_LD_XXX, F_WB_REG, N, N),
